...
const availableConnectors = ['url_batch', 'web_crawl', 'jira_project'].map(
  (id): ConnectorInfo => ({
    id,
    name: id,
    description: '',
...
describe('知识库导入入口可用状态', () => {
  ...
  it('对只读账号禁用全部导入入口', () => {
    ...
    expect(availability.filesDisabledReason).toBe('当前账号没有导入知识库的权限')
    expect(availability.urlDisabledReason).toBe('当前账号没有导入知识库的权限')
  })
    ...
    const availability = resolveKnowledgeImportAvailability({
      ...
      connectors: availableConnectors.map((connector) =>
        connector.id === 'url_batch'
          ? { ...connector, available: false, unavailable_reason: '网页导入服务未启用，请联系管理员' }
          : connector
      ),
    })
