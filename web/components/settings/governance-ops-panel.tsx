        ...
        <fieldset disabled={accessLoading} className="contents">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <ActionButton
              icon={FileClock}
              busy={busy === 'stale'}
              disabled={Boolean(busy) || !datasetId.trim()}
              label="查询待复核文档"
              onClick={() =>
                runAction('stale', '待复核文档', () =>
                  governanceApi.listStaleDocumentsByDataset(datasetId.trim(), {
                    mode,
            ...
            />
            <div className="text-xs leading-5 text-muted-foreground">每次最多返回 50 条结果。</div>
          </div>

          {errorMessage ? (
            <div
              role="alert"
              className="flex items-start gap-2 border-t border-destructive/20 pt-3 text-xs leading-5 text-destructive"
            >
          ...
          >
            <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
              ...
              <ConfirmDialog
                ...
                confirmLabel="确认删除"
                onConfirm={() =>
                  runAction('delete-preset', '删除切块预设', async () => {
              ...
              >
                <Button
                  ...
                  disabled={Boolean(busy) || !presetId.trim()}
                >
                  {busy === 'delete-preset' ? (
