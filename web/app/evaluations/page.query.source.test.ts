...
describe('evaluations page query convergence', () => {
  it('uses TanStack Query for conversations, runs, and run detail loading', () => {
    ...
    expect(src).toContain('await refreshEvaluationWorkspaceData(activeTab, {')
    expect(src).toContain('refreshConversations: () => conversationsQuery.refetch()')
    expect(src).toContain('refreshRuns: () => runsQuery.refetch()')
    expect(src).toContain('invalidateEvaluations: () =>')
    ...
    expect(src).toContain('showSummary={shouldShowConversationSummary(activeTab)}')
